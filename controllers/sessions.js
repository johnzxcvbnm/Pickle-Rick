...
//POST Routes
//Route finds the username and checks to see if hte password is correct, if so, a session is created
router.post("/", (req, res) => {
  //Find a user based off of the username.  Usernames are unique so there can only be one result or none
  User.findOne( { username: req.body.username }, (err, foundUser) => {
    //If the username was not found, send a (403: "Username not found") mesage and do nothing else
    if(foundUser === null){
      res.status(403).json({
        status: 403,
      ...
      if( bcrypt.compareSync( req.body.password, foundUser.password) ){
        ...
        //If the password is wrong send a (401: "Login Failed") message
        res.status(401).json({
          status: 401,
